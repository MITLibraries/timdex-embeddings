...
class OSNeuralSparseDocV3GTE(BaseEmbeddingModel):
    ...
    def create_embeddings(
        ...
        if num_workers > 1 or self.device == "mps":
            ...
            pool = None
        logger.info(
            f"Num workers: {num_workers}, embedding batch size: {batch_size}, "
            f"batch size: {te_batch_size}, chunk size: {chunk_size}, "
        ...
        try:
            for embedding_inputs_batch in batched(embedding_inputs, batch_size):
                ...
                logger.debug(
                    f"Batch {batch_index}: {len(embedding_inputs_batch)} records, "
